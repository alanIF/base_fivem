...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
			...
			local d2 = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),339.99853515625,-582.01806640625,28.791465759277,true)
			...
			local slot = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),-43.228378295898,-1104.5457763672,26.42234992981,true)
			...
			if d2 <= 3.0  then
			    DrawText3D(339.99853515625,-582.01806640625,28.791465759277, "PRESSIONE  ~b~E~w~  PARA BATER O CARTAO ")
				if IsControlJustPressed(0,38) then
			...
			if slot <= 3.0  then
			    DrawText3D(-43.228378295898,-1104.5457763672,26.42234992981, "PRESSIONE  ~b~E~w~  PARA COMPRAR SLOT ")
				if IsControlJustPressed(0,38) then
